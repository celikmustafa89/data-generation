...
datasetTest_AD = X_AD_tensor_folds{5};
datasetTest_MCI = X_MCI_tensor_folds{5};
...
testX = [];
testy = [];
for m=1:size(testVec_AD,1)
    testX = [testX ; testVec_AD(m,:)];
    testy = [testy ; 1];
end
for m=size(testVec_AD,1)+1:size(testVec_AD,1)+size(testVec_MCI,1)
    testX = [testX ; testVec_MCI(m-size(testVec_AD,1),:)];
    testy = [testy ; 0];
end
...
tt_AD=[];
TT = []; % all train samples
for t=1:numOfOriginalSamples
    features = [];
    for u=1:nv
        features = [features vectorize(datasetTrain_AD{u}(:,:,t))];
    end
    tt_AD = [tt_AD;features];
    TT = [TT;features];
end
tt_MCI=[];
for t=1:numOfOriginalSamples
...
for t=1:numOfNewSamples
    ...
    tt_new_mci = [tt_new_mci;features];
end

[coeffs, scores, latents,TSQUARED,explained] = pca(TT);
cof = coeffs(:,1:2);
TT_pca = TT*cof;

tt_AD_pca = tt_AD*cof;
tt_MCI_pca = tt_MCI*cof;
ts_AD_pca = ts_AD*cof;
ts_MCI_pca = ts_MCI*cof;
tt_new_ad_pca = tt_new_ad*cof;
tt_new_mci_pca = tt_new_mci*cof;

scatter(tt_AD_pca(:,1),tt_AD_pca(:,2),'filled')
hold on
scatter(tt_MCI_pca(:,1),tt_MCI_pca(:,2),'filled')

hold on
scatter(tt_new_ad_pca(:,1),tt_new_ad_pca(:,2),'filled','s')
hold on
scatter(tt_new_mci_pca(:,1),tt_new_mci_pca(:,2),'filled','s')


hold on
scatter(ts_AD_pca(:,1),ts_AD_pca(:,2),'filled','d')
hold on
scatter(ts_MCI_pca(:,1),ts_MCI_pca(:,2),'filled','d')

legend('original-AD','original-MCI','fake-AD','fake-MCI','test-AD','test-MCI')
xlabel('PCA1');
ylabel('PCA2');


% 3-d
[coeffs, scores, latents] = pca(TT);
cof = coeffs(:,1:3);
TT_pca = TT*cof;
...
scatter3(tt_AD_pca(:,1),tt_AD_pca(:,2),tt_AD_pca(:,3),'filled'); hold on;
scatter3(tt_MCI_pca(:,1),tt_MCI_pca(:,2),tt_MCI_pca(:,3),'filled'); hold on;
scatter3(tt_new_ad_pca(:,1),tt_new_ad_pca(:,2),tt_new_ad_pca(:,3),'filled','d'); hold on;
scatter3(tt_new_mci_pca(:,1),tt_new_mci_pca(:,2),tt_new_mci_pca(:,3),'filled','d'); hold on;
scatter3(ts_AD_pca(:,1),ts_AD_pca(:,2),ts_AD_pca(:,3),'s'); hold on;
scatter3(ts_MCI_pca(:,1),ts_MCI_pca(:,2),ts_MCI_pca(:,3),'s'); hold on;
