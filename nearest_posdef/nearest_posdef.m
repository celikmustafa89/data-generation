...
% the graph is removed for performance issue
back = fmincon(@distF,f,[],[],[],[],lbnd,[],@ineq);
...
diffF = in-out; %(diffF(1:size(diffF,1),1:size(diffF,2))); colorbar; colormap(copper)
%(gcf,'Color',[.97 .97 .97]); title('Difference between matrix input and output','FontSize',12)
end
